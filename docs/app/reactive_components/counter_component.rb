...
# reply.* reply styles (a flash, an in-place morph).
#
# Styling is plain daisyUI utility classes (btn) — exactly what a host app using
# the daisyui gem would write.
class CounterComponent < Phlex::HTML
  ...
  def view_template
    div(**reactive_root(class: "flex items-center gap-3")) do
      ...
      span(
        class: "text-2xl tabular-nums w-10 text-center",
        data: { testid: "count" }
      ) { @count.to_s }
