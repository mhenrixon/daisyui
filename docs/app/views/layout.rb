...
module Views
  # The site shell. A Phlex layout built on the daisyUI Drawer: a sticky topbar, a
  # sidebar always visible on desktop (lg:drawer-open) that toggles as an overlay
  # on mobile, and the page content in a scrollable main. Loads Turbo + the
  # reactive Stimulus controller (auto-pinned by the engine) and the
  # daisyUI-compiled Tailwind build.
  #
  # Layout IS the full HTML document, so controllers render it with `layout: false`
  # (see ApplicationController#render_page) to avoid double <html> nesting.
  class Layout < Phlex::HTML
    ...
    def render_head
      head do
        title { [@title, "DaisyUI Ruby"].compact.join(" · ") }
        meta(charset: "utf-8")
        ...
        # `application` is the daisyUI-compiled Tailwind build (Propshaft).
    ...
    def topbar
      div(class: "navbar bg-base-200 border-b border-base-300 sticky top-0 z-30 px-4") do
        div(class: "flex-1 items-center gap-2") do
          ...
          a(href: "/", class: "btn btn-ghost text-lg font-bold") { "DaisyUI Ruby" }
        end
