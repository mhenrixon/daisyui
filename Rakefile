...
task :release, %i[version force] do |_t, args|
  ...
  tag = "v#{new_version}"
  version_file = "lib/daisy_ui/version.rb"
  updated_at_file = "lib/daisy_ui/updated_at.rb"
  ...
  File.write(updated_at_file, <<~RUBY)
    # frozen_string_literal: true
  ...
  # Step 3: Update lockfiles and verify gem builds cleanly
